...
impl Store {
    ...
    /// Вернуть захваченное задание в очередь без признания неудачи.
    ///
    /// Нужно, когда задание пока нельзя выполнить по внешней причине: снимку
    /// территории нужен готовый фасад того же дома, а фасад ещё рисуется.
    /// Захват попыткой не считается: ожидание не должно съедать попытки,
    /// отведённые на настоящие сбои.
